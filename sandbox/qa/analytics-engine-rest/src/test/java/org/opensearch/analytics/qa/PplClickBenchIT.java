...
public class PplClickBenchIT extends AnalyticsRestTestCase {
     ...
     * <p>
     * Empty for now — PPL execution blocks on a transport thread assertion in the analytics
     * engine's DefaultPlanExecutor. Will be re-enabled once the blocking call is made async.
     */
    ...
    public void testClickBenchPplQueries() throws Exception {
        if (QUERY_NUMBERS.isEmpty()) {
            logger.info("No PPL queries configured — skipping");
            return;
        }
