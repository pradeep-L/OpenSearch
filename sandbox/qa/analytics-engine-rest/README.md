...
- `DslClickBenchIT` currently runs ClickBench Q1. Additional queries can be added as analytics-engine expands aggregation translator support and planner rules.
- `PplClickBenchIT` is scaffolded but no queries are enabled yet — the PPL path blocks on a transport thread assertion in `DefaultPlanExecutor.execute()`. Will be enabled once the analytics engine makes that call async.
