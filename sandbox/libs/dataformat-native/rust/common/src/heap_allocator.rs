...
/// Copies the name of the heap at `index` into `buf`. Returns bytes written, or -1 on error.
#[no_mangle]
pub unsafe extern "C" fn native_heap_name(index: i32, buf: *mut u8, buf_len: i32) -> i32 {
    let registry = REGISTRY.lock().unwrap();
    let i = index as usize;
    if i >= registry.len() || buf.is_null() {
        return -1;
    }
    let name = registry[i].0.as_bytes();
    let n = name.len().min(buf_len as usize);
...
pub extern "C" fn native_heap_used(index: i32) -> i64 {
    ...
    heap_stats(registry[i].1).used as i64
}
...
pub extern "C" fn native_heap_committed(index: i32) -> i64 {
    ...
    let i = index as usize;
    if i >= registry.len() { return -1; }
    heap_stats(registry[i].1).committed as i64
}
